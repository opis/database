...
class Connection
{
    ...
    public function __construct(Compiler $compiler, ConnectionInfo $info)
    {
        
        $this->name = $info->name();
        
        $this->enableLog = $info->logQueries();
        

        $options = array(
            PDO::ATTR_PERSISTENT => $info->persistent(),
        ...
        try
        {
            $this->pdo = new PDO($info->dsn(), $info->username(), $info->password(), $options);
        }
